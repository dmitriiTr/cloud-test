    ...
    {Array.from({ length: 6 }).map((_, i) => (
      <tr className="border-b" key={i}>
        {Array.from({ length: 5 }).map((_, j) => (
          <td key={j} className={`${j > 1 ? 'hidden lg:table-cell' : ''}`}>
            <div
              className={`h-4 ${i % 2 === 0 ? 'bg-gray-200' : 'bg-gray-300'} mt-3 mb-6 rounded`}
            />
          </td>
        ))}
      </tr>
...
const TableRow = observer(({ price, selectCoin }: TableRowProps) => {
  return (
    <tr
      onClick={() => selectCoin(price.symbol)}
    ...
    >
      ...
      <td
        key={price.price}
        className={`${price.change} py-4 font-medium text-black`}
      >
        {price.price}
      </td>
